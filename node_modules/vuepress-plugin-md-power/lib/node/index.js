...
import { addViteOptimizeDepsInclude, encodeData, ensureEndingSlash, ensureLeadingSlash, getFullLocaleConfig, isArray, isLinkAbsolute, isLinkHttp, isPlainObject, removeLeadingSlash } from "@vuepress/helper";
import { Buffer } from "node:buffer";
import http from "node:https";
import { URL, URLSearchParams } from "node:url";
import { attempt, camelCase, debounce, deleteKey, isBoolean, isEmptyObject, isNull, isNumber, isPlainObject as isPlainObject$1, isString, isUndefined, kebabCase, notNullish, objectMap, omit, toArray, uniqWith, withTimeout } from "@pengzhanbo/utils";
import imageSize from "image-size";
import pMap from "p-map";
import { colors, fs, getDirname, hash, logger, ora, path, tinyglobby } from "vuepress/utils";
import path$1 from "node:path";
import { ensureLeadingSlash as ensureLeadingSlash$1, isLinkHttp as isLinkHttp$1, isLinkWithProtocol, isPlainObject as isPlainObject$2, removeEndingSlash, removeLeadingSlash as removeLeadingSlash$1, resolveLocalePath } from "vuepress/shared";
...
import { removeLeadingSlash as removeLeadingSlash$2 } from "@vuepress/shared";
...
async function imageSizePlugin(app, md, type = false) {
	...
	const sizes = await getAllImageOriginalSize(await scanImage(app), type === "all");
	...
	md.renderer.rules.image = (tokens, idx, options, env, self) => {
		...
		if (width && height || !url || !sizes[url] || src?.startsWith("data:")) return imageRule(tokens, idx, options, env, self);
	...
	function createHtmlRule(rawHtmlRule) {
		return (tokens, idx, options, env, self) => {
			...
			token.content = token.content.replace(REG_IMG_TAG, (raw, info) => {
				...
				if (width && height || !url || !sizes[url] || src?.startsWith("data:")) return raw;
				const size = resolveSize(sizes[url], width, height);
				attrs.width = size.width;
				attrs.height = size.height;
				return `<img ${Object.entries(attrs).map(([key, value]) => typeof value === "boolean" ? key : `${key}="${value}"`).join(" ")}>`;
			});
	...
	*/
	function resolveSize(original, width, height) {
...
async function scanImage(app) {
	...
	await pMap(files, async (file) => {
		...
		if ((await fs.stat(filepath)).isFile() && filepath.endsWith(".md") && !filepath.includes(".vuepress") && !filepath.includes("node_modules")) {
...
*/
async function getAllImageOriginalSize(images, includeRemote = false) {
	const result = {};
...
async function getImageOriginalSize(image, includeRemote = false) {
...
*/
function resolveImagePath(app, src, currentPath) {
	if (!src) return "";
	if (isLinkHttp(src)) return src;
...
* @returns Image size / 图片尺寸
*/
async function fetchRemoteImageSize(src) {
	...
	const promise = new Promise((resolve) => {
		http.get(link, async (stream) => {
			...
			for await (const chunk of stream) {
				...
				if (data && data.width && data.height) return resolve(data);
			}
			resolve({
				width: 0,
				height: 0
	...
	try {
		return await withTimeout(() => promise, 3e3);
	} catch {}
	return {
		width: 0,
		height: 0
...
function stringifyAttrs(attrs, withUndefinedOrNull = false, forceStringify = []) {
	const result = Object.entries(attrs).map(([key, value]) => {
...
function parseFileNodes(files) {
	...
	for (const file of files) {
		const parts = removeLeadingSlash$1(file).split("/");
		let node = nodes;
		...
		before: (info, tokens, index) => {
			...
			for (let i = index + 1; !(tokens[i].nesting === -1 && tokens[i].type === "container_code-tree_close"); i++) {
				...
				if (token.type === "fence" && token.tag === "code") {
					...
					if (title) {
						files.push(title);
						if (fenceInfo.includes(":active")) activeFile = title;
					}
				}
			}
			const { attrs } = resolveAttrs(info);
...
//#endregion
//#region src/node/utils/findLocales.ts
/**
...
*/
function findLocales(locales, key) {
	const res = {};
	for (const [locale, value] of Object.entries(locales)) res[locale] = value[key] ?? {};
	return res;
}
//#endregion
...
function alignPlugin(md) {
	...
	createContainerPlugin(md, "flex", { before: (info) => {
		...
		if (attrs.column) styles.push("flex-direction:column");
		if (attrs.wrap) styles.push("flex-wrap:wrap");
...
function cardPlugin(md) {
	...
	*/
	createContainerPlugin(md, "card-masonry", {
		before: (info) => {
			const { attrs } = resolveAttrs(info);
			if (attrs.cols) attrs.cols = attrs.cols[0] === "{" ? attrs.cols : Number.parseInt(`${attrs.cols}`);
			if (attrs.gap) attrs.gap = Number(attrs.gap);
...
//#endregion
//#region src/node/utils/encryptContent.ts
/**
...
function getKeyMaterial(password) {
	...
	return webcrypto.subtle.importKey("raw", enc.encode(password), "PBKDF2", false, ["deriveBits", "deriveKey"]);
}
/**
...
async function encryptContent(content, options) {
	...
	const key = await getCryptoDeriveKey(await getKeyMaterial(password), salt);
	const enc = new TextEncoder();
	const cipherTextData = await webcrypto.subtle.encrypt({
		name: "AES-CBC",
		iv
	}, key, enc.encode(content));
...
//#endregion
//#region src/node/container/encrypt.ts
/**
...
*/
function encryptPlugin(app, md, options) {
	...
	* 将加密内容写入临时文件
	*/
	const writeTemp = async (hash, content, options) => {
	...
	*/
	...
	if (!fs.existsSync(app.dir.temp(entryFile))) app.writeTemp(entryFile, "export default {}\n");
	...
	*/
	...
	createContainerSyntaxPlugin(md, "encrypt", (tokens, index, _, env) => {
		...
		const { password, pwd, hint } = meta;
		const rendered = md.render(content, cleanMarkdownEnv(env));
		const _pwd = password || pwd || options.password;
		if (!_pwd) {
			logger$1.warn(`${colors.cyan("[encrypt snippet]")} ${colors.green("::: encrypt")} container missing password. ${colors.gray(`(${env.filePathRelative})`)}`);
			return rendered;
		}
		...
		writeEntry();
		writeTemp(contentHash, rendered, {
			salt,
			iv,
			password: String(_pwd)
		});
		return `<VPEncryptSnippet data="${encodeData(JSON.stringify({
			hash: contentHash,
			salt: Array.from(salt),
			iv: Array.from(iv)
		}))}" hint="${hint || ""}" path-locale="${getLocale(env.filePathRelative)}" />`;
	});
}
//#endregion
...
*/
function fileTreePlugin(md, options = {}, locales) {
	/**
	* Get file or folder icon
	*
	* 获取文件或文件夹的图标
...
${renderedIcon}${renderedComment}${children.length > 0 ? renderFileTree(children, meta) : ""}
	...
	return createContainerSyntaxPlugin(md, "file-tree", (tokens, index, _, env) => {
		const token = tokens[index];
		const nodes = parseFileTreeRawContent(token.content);
		const meta = token.meta;
		const cmdText = fileTreeToCMDText(nodes).trim();
		const data = locales[resolveLocalePath(locales, ensureLeadingSlash$1(env.filePathRelative || ""))] ?? {};
		return `<div class="vp-file-tree">${meta.title ? `<p class="vp-file-tree-title">${meta.title}</p>` : ""}<VPCopyButton text="${encodeData(cmdText)}" encode aria-label="${data.copy || "Copy"}" data-copied="${data.copied || "Copied"}" />${renderFileTree(nodes, meta)}</div>\n`;
	});
}
/**
...
async function langReplPlugin(app, md, { theme, go = false, kotlin = false, rust = false, python = false }) {
	...
	* 为特定语言创建容器
	*/
	const container = (lang) => createContainerPlugin(md, `${lang}-repl`, {
		before(info) {
			const { attrs } = resolveAttrs(info);
	...
	if (kotlin) container("kotlin");
	if (go) container("go");
	if (rust) container("rust");
	if (python) container("python");
	theme ??= {
		light: "github-light",
		dark: "github-dark"
	};
...
function npmToPlugins(md, options = {}) {
	...
	createContainerPlugin(md, "npm-to", {
		before: (info, tokens, idx, _opt, env) => {
			const { attrs } = resolveAttrs(info);
			const tabs = attrs.tabs ? attrs.tabs.split(/,\s*/) : defaultTabs;
			...
			if (token.type === "fence") {
				...
				token.content = "";
				const lines = content.split(/(\n|\s*&&\s*)/);
				return md.render(resolveNpmTo(lines, token.info.trim(), idx, tabs), cleanMarkdownEnv(env));
			}
...
*/
function resolveNpmTo(lines, info, idx, tabs) {
...
function validateTabs(tabs) {
	...
	if (tabs.length === 0) return DEFAULT_TABS;
	return tabs;
}
/**
	...
	createContainerSyntaxPlugin(md, "table", (tokens, index, opt, env) => {
		...
		for (const token of tableTokens) {
			...
			if (token.type === "th_open" || token.type === "td_open") {
				if (skipCells > 0) {
					if (token.type === "th_open") token.hidden = true;
					skipCells--;
					continue;
				}
				colIndex++;
				...
				if (colspanIndex >= 0) {
					...
					if (!Number.isNaN(colspanValue) && colspanValue > 1) skipCells = colspanValue - 1;
				}
			}
			if (skipCells > 0 && (token.type === "text" || token.type === "th_close" || token.type === "td_close")) {
				if (token.type === "th_close") token.hidden = true;
				continue;
			}
		}
	...
	for (let i = index + 1; i < tokens.length; i++) {
		...
		if (token.type === "bullet_list_open") {
			...
			if (listStack.length === 1) {
				...
				const softbreakIndex = inlineToken.children.findLastIndex((token) => token.type === "softbreak");
				if (softbreakIndex !== -1) {
...
function extractTimelineAttributes(rawText) {
	...
	while (buffer.length) {
		...
		attrs[matchedKey] = buffer.slice(0, valueEnd).trim().replace(RE_CLEAN_VALUE, "$2");
		buffer = buffer.slice(valueEnd);
	}
	return attrs;
}
//#endregion
//#region src/node/container/window.ts
const RE_IMAGE_SYNTAX = /^!?\[[^\]]*\]\([^)]+\)$/;
const render = (info, tokens, idx) => {
	const elms = [];
	for (let i = idx + 1; i < tokens.length; i++) {
		if (tokens[i].type === "container_window_close") break;
		elms.push(tokens[i]);
	}
	const { attrs } = resolveAttrs(info);
	let onlyImg = false;
	if (elms.length === 1) {
		...
		if (type === "html_block" && (content.startsWith("<img") || content.startsWith("<picture"))) onlyImg = true;
	}
	if (elms.length === 3) {
		const [op, img, cp] = elms;
		if (op.type === "paragraph_open" && cp.type === "paragraph_close" && img.type === "inline" && RE_IMAGE_SYNTAX.test(img.content.trim())) {
			op.type = "text";
			cp.type = "text";
			onlyImg = true;
		}
	...
	const gap = isString(attrs.gap) || isNumber(attrs.gap) ? parseRect(attrs.gap) : onlyImg || noPadding ? "0" : "20px";
	const classes = ["window-wrapper"];
...
function windowPlugin(md) {
	...
	createContainerPlugin(md, "demo-wrapper", {
		before: (info, tokens, idx, _, env) => {
			logger$1.warn("container", `::: demo-wrapper container is deprecated, please use ::: window container instead. (${colors.gray(env.filePathRelative || "")})`);
...
* @param locales - Locale configuration / 本地化配置
*/
async function containerPlugin(app, md, options, locales) {
	alignPlugin(md);
	tabs(md);
	...
	stepsPlugin(md);
	cardPlugin(md);
	if (options.encrypt) encryptPlugin(app, md, typeof options.encrypt === "boolean" ? {} : options.encrypt);
	...
	if (options.fileTree) fileTreePlugin(md, isPlainObject(options.fileTree) ? options.fileTree : {}, findLocales(locales, "common"));
...
function markdownEmbed(app, md, env, { url, title, desc, codeSetting = "", expanded = false }) {
	...
	if (code === false) {
		logger$1.warn("demo-markdown", `Cannot read markdown file: ${colors.gray(filepath)}\n  at: ${colors.gray(env.filePathRelative || "")}`);
		return "";
	}
	const demo = {
		type: "markdown",
	...
	env.demoFiles ??= [];
	if (!env.demoFiles.some((d) => d.path === filepath)) env.demoFiles.push(demo);
...
const SCRIPT_RE$1 = /<script\b[^>]*>/;
function insertSetupScript({ export: name, path }, env) {
...
function demoWatcher(app, watchers) {
	...
	watcher.on("change", (path) => {
		if (tasks[path]) {
			...
			compileCode(parseEmbedCode(code), tasks[path]);
		}
	});
...
function addTask(app, path, output) {
	if (tasks[path]) return;
	tasks[path] = output;
	if (watcher) watcher.add(path);
	updateWatchFiles(app);
}
...
function normalEmbed(app, md, env, { url, title, desc, codeSetting = "", expanded = false }) {
	...
	const code = readFileSync(filepath);
	if (code === false) {
		logger$1.warn("demo-normal", `Cannot read demo file: ${colors.gray(filepath)}\n  at: ${colors.gray(env.filePathRelative || "")}`);
		return "";
	}
	const source = parseEmbedCode(code);
	const prefix = (env.filePathRelative || "").replace(/\.md$/, "").replace(/\//g, "-");
	const basename = path$1.basename(filepath).replace(/-|\./g, "_");
...
function vueEmbed(app, md, env, { url, title, desc, codeSetting = "", expanded = false }) {
	...
	const code = readFileSync(filepath);
	if (code === false) {
		logger$1.warn("demo-vue", `Cannot read vue demo file: ${colors.gray(filepath)}\n  at: ${colors.gray(env.filePathRelative || "")}`);
		return "";
	}
	const basename = path$1.basename(filepath).replace(/-|\./g, "_");
	const ext = path$1.extname(filepath).slice(1);
	const name = `Demo${basename[0].toUpperCase()}${basename.slice(1)}`;
	const demo = {
		type: "vue",
		export: name,
...
function transformImports(code, filepath) {
	return code.replace(IMPORT_RE, (matched, url) => {
		if (url.startsWith("./") || url.startsWith("../")) return matched.replace(url, `${path$1.resolve(path$1.dirname(filepath), url)}`);
		return matched;
	});
}
...
function extendsPageWithDemo(page) {
	...
	page.deps.push(...demoFiles.filter(({ type }) => type === "markdown").map(({ path }) => path));
	(page.frontmatter.gitInclude ??= []).push(...demoFiles.filter(({ gitignore }) => !gitignore).map(({ path }) => path));
}
//#endregion
...
const codepenPlugin = (md) => {
	createEmbedRuleBlock(md, {
		...
		meta: ([, info, source]) => {
			const { width, height, title, tab, preview, editable, theme } = resolveAttrs(info).attrs;
...
const jsfiddlePlugin = (md) => {
	createEmbedRuleBlock(md, {
		...
		meta([, info, source]) {
			const { width, height, title, tab, theme } = resolveAttrs(info).attrs;
			return {
				width: width ? parseRect(width) : "100%",
				height: height ? parseRect(height) : "400px",
				source,
	...
	createEmbedRuleBlock(md, {
		...
		meta([, info, id]) {
			...
			return {
				...
				width: attrs.width ? parseRect(attrs.width) : "100%",
				height: attrs.height ? parseRect(attrs.height) : void 0,
	...
	createEmbedRuleBlock(md, {
		...
		meta([, info, source]) {
			...
			return {
				...
				volume: typeof attrs.volume !== "undefined" ? Number(attrs.volume) : .75,
				poster: attrs.poster,
				width: attrs.width ? parseRect(attrs.width) : "100%",
				height: attrs.height ? parseRect(attrs.height) : void 0,
				ratio: attrs.ratio ? parseRect(`${attrs.ratio}`) : void 0
			};
		},
		content({ url, ...meta }) {
...
function resolvePaths(rawPath, base, filePathRelative) {
	...
	if (rawPath.startsWith("/")) if (rawPath.endsWith(".md")) {
		...
		relativePath = removeLeadingSlash$2(rawPath);
	} else {
...
function resolveIcon(content, options) {
	...
	const info = [];
	const excludes = [];
	for (const key in attrs) if (attrs[key] === true) {
		excludes.push(key);
...
function prepareIcon(imports, options = {}) {
	...
	if (options.provider === "iconfont") assets.push(...toArray(options.assets).map((asset) => normalizeAsset(asset)).filter(notNullish));
		...
		if (asset === "fontawesome") return [
			...
			"regular",
			"fontawesome"
		].map(getFontAwesomeCDNLink).map((asset) => normalizeAsset(asset, "fontawesome"));
	...
	let hasStyle = false;
	let hasScript = false;
	for (const asset of uniqWith(assets, (a, b) => a.link === b.link)) if (asset.type === "style") {
		hasStyle = true;
...
*/
const abbrPlugin = (md, globalAbbreviations = {}) => {
...
*/
const annotationPlugin = (md, globalAnnotations = {}) => {
...
//#endregion
//#region src/node/inline/env-preset.ts
/**
...
*/
const envPresetPlugin = (md, env = {}) => {
	if (isEmptyObject(env)) return;
 ...
 /**
	...
	*/
	md.use(annotationPlugin, {
		...env.annotations,
		...isPlainObject$1(options.annotation) ? options.annotation : {}
	});
	if (options.abbr)
 /**
	* a HTML element
	...
	*/
	md.use(abbrPlugin, {
		...env.abbreviations,
		...isPlainObject$1(options.abbr) ? options.abbr : {}
	});
	if (options.plot === true || isPlainObject$1(options.plot)) md.use(plotPlugin);
}
//#endregion
...
//#endregion
//#region src/node/obsidian/comment.ts
function commentPlugin(md) {
	md.inline.ruler.before("html_inline", "obsidian_inline_comment", (state, silent) => {
		let found = false;
		const max = state.posMax;
		const start = state.pos;
		if (state.src.charCodeAt(start) !== 37 || state.src.charCodeAt(start + 1) !== 37) return false;
		/* istanbul ignore if -- @preserve */
		if (silent) return false;
		if (max - start < 5) return false;
		state.pos = start + 2;
		while (state.pos < max) {
			if (state.src.charCodeAt(state.pos) === 37 && state.src.charCodeAt(state.pos + 1) === 37) {
				found = true;
				break;
			}
			state.md.inline.skipToken(state);
...
//#endregion
//#region src/node/obsidian/findFirstPage.ts
const pagePaths = [];
function initPagePaths(app) {
	pagePaths.length = 0;
...
function updatePagePaths(filepath, type) {
	if (!filepath) return;
	if (type === "create") pagePaths.push(filepath);
	if (type === "delete" && pagePaths.includes(filepath)) pagePaths.splice(pagePaths.indexOf(filepath), 1);
	sortPagePaths();
}
function findFirstPage(filename, currentPath) {
...
//#endregion
//#region src/node/obsidian/embedLink.ts
...
const EXTENSION_VIDEOS = SUPPORTED_VIDEO_TYPES.map((ext) => `.${ext}`);
const embedLinkDef = (state, startLine, endLine, silent) => {
	const start = state.bMarks[startLine] + state.tShift[startLine];
	const max = state.eMarks[startLine];
	if (max - start < 6) return false;
	if (state.src.charCodeAt(start) !== 33 || state.src.charCodeAt(start + 1) !== 91 || state.src.charCodeAt(start + 2) !== 91) return false;
	const line = state.src.slice(start, max).trim();
	if (line.charCodeAt(line.length - 1) !== 93 || line.charCodeAt(line.length - 2) !== 93) return false;
	/* istanbul ignore if -- @preserve */
	if (silent) return true;
	...
	if (EXTENSION_IMAGES.includes(extname)) {
		...
		if (settings) {
			...
			const styles = [];
			if (width) styles.push(`width: ${parseRect(width)}`);
			if (height) styles.push(`height: ${parseRect(height)}`);
		...
		state.push("source_open", "source", 1).attrSet("src", resolveFilenameToAssetPath(filename));
	...
	} else if (isLinkHttp$1(filename) || extname && extname !== ".md") {
...
function embedLinkPlugin(md, app) {
	md.block.ruler.before("import_code", "obsidian_embed_link", embedLinkDef, { alt: [
		"paragraph",
		"reference",
		"blockquote",
		"list"
	...
	md.renderer.rules.obsidian_embed_link = (tokens, idx, _, env) => {
		const { filename, hashes, settings } = tokens[idx].meta;
		...
		const url = ensureLeadingSlash$1(filename[0] === "." ? path.join(path.dirname(env.filePathRelative ?? ""), filename) : filename);
		const anchor = hashes.at(-1);
		const slug = anchor ? `#${slugify(anchor)}` : "";
		const text = settings || filename + (hashes.length ? ` > ${hashes.join(" > ")}` : "");
		return `<a href="${url}${slug}" target="_blank" rel="noopener noreferrer">${md.utils.escapeHtml(text)}</a>`;
	};
}
function resolveFilenameToAssetPath(filename) {
	if (isLinkHttp$1(filename) || filename[0] === "." || filename[0] === "/") return filename;
	return `/${filename}`;
}
const HEADING_HASH_REG = /^#+/;
const HEADING_ATTRS_REG = /(?:\{[^}]*\})?$/;
function extractContentByHeadings(content, headings) {
	...
	for (let i = 0; i < allHeadings.length; i++) {
		...
		} else if (heading.level > currentLevel && heading.text === headings[headingPointer]) {
			headingPointer++;
			currentLevel = heading.level;
			if (headingPointer === headings.length) {
				targetHeadingIndex = i;
		...
		} else if (heading.level <= currentLevel) if (heading.text === headings[0]) {
			headingPointer = 1;
			currentLevel = heading.level;
		} else {
			headingPointer = 0;
	...
	for (let i = targetHeadingIndex + 1; i < allHeadings.length; i++) if (allHeadings[i].level <= targetLevel) {
		endLine = allHeadings[i].lineIndex;
		break;
	}
	return lines.slice(startLine, endLine).join("\n").trim().replaceAll(/<!--container:(.*?)-->/g, (_, key) => containers[key] ?? "");
}
//#endregion
//#region src/node/obsidian/wikiLink.ts
const wikiLinkDef = (state, silent) => {
	let found = false;
	const max = state.posMax;
	const start = state.pos;
	if (state.src.charCodeAt(start) !== 91 || state.src.charCodeAt(start + 1) !== 91) return false;
	/* istanbul ignore if -- @preserve */
	if (silent) return false;
	if (max - start < 5) return false;
	state.pos = start + 2;
	while (state.pos < max) {
		if (state.src.charCodeAt(state.pos) === 93 && state.src.charCodeAt(state.pos + 1) === 93) {
			found = true;
			break;
		}
		state.md.inline.skipToken(state);
...
function wikiLinkPlugin(md) {
	...
	md.renderer.rules.obsidian_wiki_link = (tokens, idx, _, env) => {
		const { filename, titles, alias } = tokens[idx].meta;
		const anchor = titles.at(-1);
		const slug = anchor ? `#${slugify(anchor)}` : "";
		if (isLinkHttp$1(filename)) {
			const text = alias || filename + (titles.length ? ` > ${titles.join(" > ")}` : "");
			return `<a href="${filename}${slug}" target="_blank" rel="noopener noreferrer">${md.utils.escapeHtml(text)}</a>`;
		}
		if (!filename) return `<VPLink href="${slug}">${md.utils.escapeHtml(alias) || (titles.length ? `<template #after-text>${md.utils.escapeHtml(` > ${titles.join(" > ")}`)}</template>` : "")}</VPLink>`;
		const pagePath = findFirstPage(filename, env.filePathRelative ?? "");
		if (pagePath) {
			const { absolutePath, relativePath } = resolvePaths(pagePath, env.base || "/", env.filePathRelative ?? null);
			(env.links ??= []).push({
				raw: pagePath,
				absolute: absolutePath,
				relative: relativePath
			});
			return `<VPLink href="${ensureLeadingSlash$1(pagePath)}${slug}">${md.utils.escapeHtml(alias) || (titles.length ? `<template #after-text>${md.utils.escapeHtml(` > ${titles.join(" > ")}`)}</template>` : "")}</VPLink>`;
		}
		const url = ensureLeadingSlash$1(filename[0] === "." ? path.join(path.dirname(env.filePathRelative ?? ""), filename) : filename);
		const text = alias || filename + (titles.length ? ` > ${titles.join(" > ")}` : "");
		return `<a href="${url}${slug}" target="_blank" rel="noopener noreferrer">${md.utils.escapeHtml(text)}</a>`;
...
//#endregion
//#region src/node/obsidian/index.ts
function obsidianPlugin(app, md, options) {
	if (options.obsidian === false) return;
	const obsidian = isPlainObject$2(options.obsidian) ? options.obsidian : {};
	initPagePaths(app);
	if (obsidian.wikiLink !== false) wikiLinkPlugin(md);
	if (obsidian.embedLink !== false) embedLinkPlugin(md, app);
	if (obsidian.comment !== false) commentPlugin(md);
}
//#endregion
...
* ```
*/
function provideData(options, locales) {
...
function markdownPowerPlugin(options = {}) {
	return (app) => {
		...
		return {
			...
			alias: (_, isServer) => {
				if (!isServer) return { ...options.encrypt ? { "/^vue$/": "vue/dist/vue.esm-bundler.js" } : void 0 };
				return {};
			},
			...
			onPageUpdated(_app, type, newPage, oldPage) {
				if (type === "create") updatePagePaths(newPage?.filePathRelative ?? "", "create");
				if (type === "delete") updatePagePaths(oldPage?.filePathRelative ?? newPage?.filePathRelative ?? "", "delete");
			}
		};
	};
}
//#endregion
export { createCodeTabIconGetter, getImageOriginalSize, markdownPowerPlugin, resolveImagePath };
