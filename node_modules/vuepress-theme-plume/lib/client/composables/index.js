...
//#endregion
//#region src/client/composables/layout.ts
...
function registerWatchers() {
	...
	if (inBrowser) watch(lang, (newLang) => {
...
function useLink(href, target) {
	...
	const maybeIsExternal = computed(() => {
		...
		if (rawTarget === "_blank" || isLinkExternal(link)) return true;
		return false;
	});
	const preProcessLink = computed(() => {
		const link = toValue(href);
		if (!link || maybeIsExternal.value) return link;
		if (link[0] === "#") return page.value.path + link;
		const path = resolveRouteFullPath(link, page.value.filePathRelative ? `/${page.value.filePathRelative}` : void 0);
		if (path.includes("#")) {
...
function resolveNavbar(navbar, _prefix = "") {
	...
	navbar.forEach((item) => {
		if (typeof item === "string") resolved.push(resolveNavLink(normalizeLink(_prefix, item)));
			...
			if (items?.length) res.items = resolveNavbar(items, normalizeLink(_prefix, prefix));
			resolved.push(res);
		}
	});
	return resolved;
...
function sortCategory(items) {
	for (const item of items) if (item.type === "category" && item.items.length) item.items = sortCategory(item.items);
...
function usePrevNext() {
	...
	return {
		prev: computed(() => {
			...
			else return resolveFromSidebarItems(flatSidebar(sidebar.value), route.path, -1);
		}),
		next: computed(() => {
...
function resolveFromSidebarItems(sidebarItems, currentPath, offset) {
	...
	while ((index += offset) >= 0 && index < sidebarItems.length) {
		...
		if (targetItem?.link && !SEPARATOR_RE.test(targetItem.link)) return targetItem;
	}
	return null;
}
