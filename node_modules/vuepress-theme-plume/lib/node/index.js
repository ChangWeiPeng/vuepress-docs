...
import { Logger, addViteConfig, addViteOptimizeDepsExclude, addViteOptimizeDepsInclude, addViteSsrNoExternal, chainWebpack, ensureEndingSlash, ensureLeadingSlash, entries, fromEntries, getFullLocaleConfig, isArray as isArray$1, isBoolean, isFunction, isLinkAbsolute, isLinkHttp, isLinkWithProtocol, isPlainObject as isPlainObject$1, removeLeadingSlash } from "@vuepress/helper";
...
function extendsBundlerOptions(bundlerOptions, app) {
	...
	addViteConfig(bundlerOptions, app, { css: { preprocessorOptions: {
...
function detectDependencies(options, plugins) {
	...
	[
		...
		if (markdown[dep] || mdEnhance[dep]) add(dep);
	});
...
function autoCategory(page, options) {
	...
	LOCALE_RE ??= new RegExp(`^(${Object.keys(options.locales || {}).filter((l) => l !== "/").join("|")})`);
...
function markdownPlugins(pluginOptions) {
	...
	if (pluginOptions.markdownEnhance !== false) plugins.push(mdEnhancePlugin({
...
function splitMarkdownOptions(options) {
	...
	const mdPower = {};
	for (const key in restOptions) if (MARKDOWN_ENHANCE_FIELDS.includes(key)) mdEnhance[key] = restOptions[key];
