import { deepMerge, deleteKey, difference, hasOwn, isArray, isEmptyObject, isFunction, isNumber, isPlainObject, isString, kebabCase, omit, sleep, toArray, uniq } from "@pengzhanbo/utils";
...
async function compiler(configPath) {
	...
	return {
		config,
		dependencies: Object.keys(result.metafile?.inputs ?? {}).filter((dep) => dep[0] === ".").map(normalizePath$1)
	};
}
//#endregion
...
function getSidebarLink(items, link, dir = "", res = {}) {
	...
	for (const item of items) if (typeof item === "string") res[ensureEndingSlash$1(dir)] = link;
	else {
...
async function generateWithDoc(data, context, collection, fm, locale) {
	...
	if (ep && !hasOwn(data, "permalink")) if (isRoot) data.permalink = path.join(locale, collection.linkPrefix, "/");
		...
		const link = resolveLinkBySidebar(collection.sidebar, ensureLeadingSlash$1(collection.dir))[ensureLeadingSlash$1(ensureEndingSlash$1(path.dirname(context.relativePath)))] || "/";
...
function getAutoDirSidebar(app, prefix) {
	...
	for (const page of pages) {
		...
		while (dir = paths[index]) {
			...
			if (dir.endsWith(".md")) if (isHome) if (parent) parent.link = path;
			else rootLink = path;
			else {
				current.link = path;
				current.text = title;
...
function cleanSidebar(sidebar) {
	for (const item of sidebar) if (isPlainObject$1(item)) {
		if (isArray$1(item.items)) if (item.items.length === 0) deleteKey(item, ["items", "collapsed"]);
...
function getAllSidebar() {
	...
	for (const [locale, opt] of entries(options.locales || {})) {
		...
		locales[locale] = {};
		for (const [key, value] of entries(sidebar || {})) locales[locale][ensureLeadingSlash(key)] = isPlainObject$1(value) && "items" in value ? {
			...value,
